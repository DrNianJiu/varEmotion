...
class UViT_Fig(nn.Module):
    def __init__(self, img_size=28, in_chans=1, patch_size=2, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
                 qkv_bias=False, qk_scale=None, norm_layer=nn.LayerNorm, mlp_time_embed=False,
                 num_classes=10, num_cond_token=1, conv=True, skip=True):
        super().__init__()
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        ...
        self.context_embed = nn.Embedding(num_classes, embed_dim)
        ...
        self.norm = norm_layer(embed_dim)
        self.patch_dim = self.patch_dim = patch_size ** 2 * in_chans
        self.decoder_pred = nn.Linear(embed_dim, self.patch_dim, bias=True)
    ...
    def forward(self, x, timesteps, context=None):
        ...
        x = self.norm(x)
        x = self.decoder_pred(x)#(B ,L+2 ,patch_size**2*in_chans)
        x = x[:, :L, :] #(B ,L ,patch_size**2*in_chans)
        # print(x.shape)
        c = self.in_chans
        p = self.patch_size
...
if __name__ == "__main__":
    ...
    x_fig = torch.rand(1,1,28,28)
    t_fig = torch.randint(1000, (1, ))
    c_fig = torch.zeros(1, 10)
    c_fig[:,0] = c_fig[:,0] + 1
    print(c_fig)
    # tokenizer = PatchEmbed(img_size=28,patch_size=2,in_chans=1)
    # print(tokenizer(x).shape)
    fig_net = UViT_Fig(num_classes=10)
    print(fig_net(x_fig,t_fig,c_fig).shape)