...
class PriorPipe:
    ...
    def train(self, dataloader, path=None, num_epochs=10, learning_rate=1e-4, uncondition_rate=0.1, is_unetcond2d=False):
        ...
        for epoch in range(num_epochs):
            ...
            for batch in dataloader:
                ...
                # print(c_embeds)
                N = h_embeds.shape[0]
                if not is_unetcond2d:
                    # 1. randomly replace c_embeds by None
                ...
                # 5. predict noise
                # noise_pre = self.diffusion_prior(perturbed_h_embeds, timesteps, context=None)
                if is_unetcond2d:
                    noise_pre = self.diffusion_prior(perturbed_h_embeds,timesteps,encoder_hidden_states=None,class_labels=c_embeds).sample
                else:
    ...
    def generate(
        ...
        for _, t in tqdm(enumerate(timesteps)):
            ...
            if guidance_scale == 0 or c_embeds is None:
                if is_unetcond2d:
                    noise_pred = model(h_t, t_tensor,encoder_hidden_states=None).sample
                else:
                    noise_pred = model(h_t, t_tensor, c_embeds)
            else:
                if is_unetcond2d:
                    noise_pred_cond = model(h_t, t_tensor,encoder_hidden_states=None,class_labels=c_embeds).sample
                    # noise_pred_uncond = model(h_t, t_tensor,encoder_hidden_states=None).sample
                    noise_pred = noise_pred_cond
                else:
                    noise_pred_cond = model(h_t, t_tensor, c_embeds)
                    noise_pred_uncond = model(h_t, t_tensor, None)
    ...
    def train_unet_mix(self, dataloader, path=None, num_epochs=10, learning_rate=1e-4, uncondition_rate=0.1):
        ...
        for epoch in range(num_epochs):
            ...
            for batch in dataloader:
                ...
                # 5. predict noise
                # noise_pre = self.diffusion_prior(perturbed_h_embeds, timesteps, context=None)
                noise_pre = self.diffusion_prior(perturbed_h_embeds, timesteps, None).sample
                # 6. loss function weighted by sigma
                loss = criterion(noise_pre, noise) # (batch_size,)
                loss = loss.mean()
    ...
    def generate_unet_mix(
        ...
        for _, t in tqdm(enumerate(timesteps)):
            ...
            # 4.1 noise prediction
            if guidance_scale == 0 or c_embeds is None:
                noise_pred = model(h_t, t_tensor, None).sample
            else:
                noise_pred_cond = model(h_t, t_tensor, None).sample
                noise_pred_uncond = model(h_t, t_tensor, None).sample
    ...
    def generate_unet(
        ...
        for _, t in tqdm(enumerate(timesteps)):
            ...
            #     noise_pred_cond = model(h_t, t_tensor, None).sample
            #     noise_pred_uncond = model(h_t, t_tensor, None).sample
            noise_pred = model(h_t,t_tensor).sample
            # perform classifier-free guidance
            # noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)
